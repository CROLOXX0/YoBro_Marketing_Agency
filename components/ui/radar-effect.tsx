      ...
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(255, 146, 28, ${0.4 - (idx + 1) * 0.04})`, // using primary color
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-glass-stroke bg-surface-container shadow-[0_0_15px_rgba(255,146,28,0.15)] group-hover:scale-110 transition-transform">
        {icon || (
      ...
      <div className="mt-2 rounded-md px-2 py-1 bg-surface-container-high/80 backdrop-blur-sm border border-glass-stroke">
        <div className="text-center text-[10px] font-bold text-crisp-white uppercase tracking-wider whitespace-nowrap">
          {text || "Service"}
        </div>
      </div>
    </motion.div>
